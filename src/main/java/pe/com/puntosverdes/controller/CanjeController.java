...
public class CanjeController {
    ...
    public ResponseEntity<?> solicitarCanje(@RequestBody CanjeSolicitudDTO dto) {
        ...
        notificacionService.crearNotificacion(
                new pe.com.puntosverdes.model.Notificacion(
                        "Canje solicitado",
                        "Tu canje de '" + recompensa.getNombre() + "' fue enviado y está pendiente de revisión.",
                        usuario,
                        false
                )
        );
    ...
    public ResponseEntity<?> resolverCanje(@PathVariable Long canjeId, @RequestBody CanjeResolucionDTO dto) {
        ...
        if (dto.isAceptado()) {
            ...
            notificacionService.crearNotificacion(
                    new pe.com.puntosverdes.model.Notificacion(
                            "Canje aprobado",
                            "Tu canje de '" + canje.getRecompensa().getNombre() +
            ...
            notificacionService.crearNotificacion(
                    new pe.com.puntosverdes.model.Notificacion(
                            "Canje rechazado",
                            "Tu canje de '" + canje.getRecompensa().getNombre() +
                            "' fue rechazado. Motivo: " + dto.getMotivo() +
                            " | Mensaje: " + dto.getRespuestaAdmin(),
                            canje.getUsuario(),
                            false
                    )
            );
