...
public class UsuarioController {
	...
	public ResponseEntity<List<UsuarioDTO>> buscarPorCelular(@PathVariable String celular) {
		List<UsuarioDTO> usuarios = usuarioService.obtenerUsuariosPorCelular(celular).stream()
				.map(usuarioService::convertirADTO).collect(Collectors.toList());
		return ResponseEntity.ok(usuarios);
	}
	...
	public ResponseEntity<UsuarioPerfilDTO> miPerfil(Authentication auth) {
		...
		UsuarioPerfilDTO dto = new UsuarioPerfilDTO(usuario.getId(), usuario.getUsername(), usuario.getNombre(),
				usuario.getApellido(), usuario.getEmail(), usuario.getCelular(), usuario.getPerfil(), roles,
				usuario.getPuntosAcumulados(), usuario.getFechaRegistro(), usuario.isEnabled());
	...
	@PostMapping(path = "/{id}/perfil", consumes = "multipart/form-data")
	...
	@GetMapping("/ranking/mi")
	public ResponseEntity<Map<String, Object>> miRanking(Authentication auth) {
	    Usuario usuario = usuarioService.obtenerUsuarioPorUsername(auth.getName());
	    List<Usuario> ranking = usuarioService.obtenerRankingUsuarios();

	    int posicion = -1;
	    for (int i = 0; i < ranking.size(); i++) {
	        if (ranking.get(i).getId().equals(usuario.getId())) {
	            posicion = i + 1;
	            break;
	        }
	    }
	    ...
	    Map<String, Object> resultado = Map.of(
	        ...
	        "puntos", usuario.getPuntosAcumulados(),
	        "posicion", posicion,
	...
	public ResponseEntity<UsuarioDTO> actualizarUsuarioPorAdmin(@PathVariable Long id,
